...
# FIXED: Changed from 'kill 0' to 'cleanup' for safety
trap cleanup EXIT
...
    AWK_BIN="gawk"
  else
    # FIXED: Improved error message for Mint/Debian users
    error_exit "GNU awk required (supports RS='\\0'). Install with: sudo apt update && sudo apt install gawk"
  fi
  
...
    
    # FIXED: Correctly parse the array element, not an undefined 'line' variable
    local path size mtime
...
  check_dependencies
 
  # FIXED: If fast mode is enabled with a destructive action, auto-enable verify mode for safety
